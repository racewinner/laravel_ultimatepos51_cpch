...
class ZoneController extends Controller
{
    ...
    public function index() {
        // if (! auth()->user()->can('zone.permission')) {
        ...
        if (request()->ajax()) {
            $rows = Zone::where('business_id', $business_id);
            $output = Datatables::of($rows)
                ->addColumn('action', function($row) {
                    ...
                    $html .= '<li class="btn btn-primary px-4 py-1">'
                            ...
                            . action([\Modules\Partner\Http\Controllers\ZoneController::class, 'edit'], [$row->id])
                            . '" class="btn-modal" data-container=".zone-modal">'
                            . '<i class="fas fa-edit" style="margin-right: 5px !important;" aria-hidden="true"></i>'
                            . __('messages.edit').'</a>
                    </li>';
                    
                    $html .= '<li class="btn btn-danger px-4 py-1 ms-4">'
                        . '<a class="delete-zone" style="color:white !important;" href="'
                        . action([\Modules\Partner\Http\Controllers\ZoneController::class, 'destroy'], [$row->id])
        ...
        return view("partner::zone.index");
    }
    ...
    public function create() {
        ...
        $zone = null;
        return view("partner::zone.edit", compact('zone'));
    }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            // To check whether the same zone exists.
            $exist = Zone::where('name', $data['name'])->where('business_id', $business_id)->first();
            if(!empty($exist)) {
    ...
    public function update($id) {
        ...
        try {
            ...
            // To check if the same zone already exists.
            $exist = Zone::where('business_id', $business_id)
                ->where('id', '!=', $id)
    ...
    public function destroy($id) {
        ...
        try {
            ...
            $count = Partner::where('zone_id', $id)->count();
            if($count > 0) {
                return response()->json([
                    'success' => 0,
