...
function initMonthPicker(selector, options) {
    $(selector).monthrangepicker({
        startDate: options?.startMonth,
        endDate: options?.endMonth,
        locale: {
            format: 'MM/YYYY'
        },
        ...
        isInvalidDate: function (date) {
            return date.date() !== 1; // Disable all days except the first day of the month
        }
    }, function (start, end) {
        options?.onChange && options.onChange(start.format('MM/YYYY'), end.format('MM/YYYY'));
        // $('#monthRangePicker').val(start.format('MM/YYYY') + ' - ' + end.format('MM/YYYY'));
    });
}

function toastrEx(type, message, title) {
...
function toastrSwal(message, type="success", callback) {
    swal({
        text: message,
        ...
        dangerMode: type != 'success',
    }).then((value) => {
        if(callback) callback(value);
    })
}

function toastrSwalArray(msgArray, callback) {
    if(msgArray?.length > 0) {
        const message = msgArray[0].message;
        const type = msgArray[0].type.toLowerCase();
        swal({
            text: message,
            icon: type,
            buttons: {
            ...
            msgArray.splice(0, 1);
            if(msgArray.length > 0) {
                toastrSwalArray(msgArray, callback);
            } else {
                if(callback) callback(value);
            }
        })  
