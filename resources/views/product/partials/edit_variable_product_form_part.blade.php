<div class="col-sm-12">
  <h4>Add Variation:*
    <button type="button" class="btn btn-primary" id="add_variation" data-action="edit">+</button>
  </h4>
</div>
<div class="col-sm-12">
      ...
      <tbody>
        @foreach($product_variations as $product_variation)
          ...
          <tr class="variation_row">
            <td>
            {!! Form::text(
        'product_variation[' . $loop->index . '][name]',
        ...
        ['class' => 'form-control input-sm variation_name', 'required']
            ...
            <table class="table table-condensed table-bordered blue-header variation_value_table">
              ...
              <th>@lang('product.default_purchase_price') &nbsp;&nbsp;<b><i class="fa fa-info-circle"
                  aria-hidden="true" data-toggle="popover" data-html="true" data-trigger="hover"
                  data-content="<p class='text-primary'>Drag the mouse over the table cells to copy input values</p>"
                  data-placement="top"></i></b></th>
              <th>@lang('product.default_selling_price') &nbsp;&nbsp;<b><i class="fa fa-info-circle"
                  aria-hidden="true" data-toggle="popover" data-html="true" data-trigger="hover"
                  data-content="<p class='text-primary'>Drag the mouse over the table cells to copy input values</p>"
                  data-placement="top"></i></b></th>
              ...
              <td>
              {!! Form::text(
            'product_variation[' . $loop->parent->index . '][variations][' . $loop->index . '][value]',
            $variation->name,
            ['class' => 'form-control input-sm variation_value_name', 'required']
          ) !!}
              {!! Form::hidden('product_variation[' . $loop->parent->index . '][variations][' . $loop->index . '][variation_id]', $variation->id) !!}
              </td>
              <td class="drag-select">
              {!! Form::number(
            'product_variation[' . $loop->parent->index . '][variations][' . $loop->index . '][default_purchase_price]',
            $variation->default_purchase_price,
            ['class' => 'form-control input-sm dpp', 'min' => '0']
          ) !!}
              </td>
              <td class="drag-select">
              {!! Form::number(
            'product_variation[' . $loop->parent->index . '][variations][' . $loop->index . '][default_sell_price]',
            $variation->default_sell_price,
            ['class' => 'form-control input-sm variable_dsp', 'min' => '0', 'placeholder' => __('product.exc_of_tax')]
          ) !!}
              {!! Form::number(
            'product_variation[' . $loop->parent->index . '][variations][' . $loop->index . '][sell_price_inc_tax]',
            $variation->sell_price_inc_tax,
            ['class' => 'form-control input-sm variable_dsp_inc_tax', 'min' => '0', 'placeholder' => __('product.inc_of_tax')]
          ) !!}
              </td>
